       ...
       .controller('MainController', ['$scope', '$http', '$filter', '$uibModal', function ($scope, $http, $filter, $uibModal) {
                 ...
                 //  alert("Por favor establecer texto de busqueda");
                   ...
                   if ($scope.tipoDocumento == 1) {
                       ...
                       $http({
                           ...
                           data: { dsc_numero_pase: $scope.filtro, cod_puesto: $scope.puestoSeleccionado.cod_atributo }
                       }).then(function mySucces(response) {
                           $scope.persona = response.data.d;
           ...
           $scope.obtenerVisitasFacilidades = function () {
               ...
               $http({
                   ...
                   data: { as_cod_puesto: $scope.puestoSeleccionado.cod_atributo, as_TipoAut: $scope.tipoIngreso, as_FiltroOpc: $scope.tipoDocumento, as_textoBus: $scope.filtro }
               }).then(function mySucces(response) {
           ...
           $scope.obtenerIngresosSalidas = function () {
               ...
               $http({
                   ...
                   data: { cod_autorizacion: $scope.visitaSeleccionada.cod_autorizacion, tipo_autorizacion: $scope.tipoIngreso, opcion_busqueda: $scope.tipoDocumento, filtro: $scope.filtro }
               }).then(function mySucces(response) {
           ...
           $scope.obtenerAccesos = function (visitaSeleccionada) {
               ...
               $http({
                   ...
                   data: { cod_visitaFacilidad: $scope.visitaSeleccionada.cod_autorizacion, cod_puesto: $scope.puestoSeleccionado.cod_atributo }
               }).then(function mySucces(response) {
           ...
           $scope.buscarPersona = function (event) {
               ...
               $http({
                   ...
                   data: { dsc_apellido_nombre: $scope.filtroPersona, cod_proceso: 335, cod_puesto: codPuesto, cod_tipoF: codTipoFacilidad }
               }).then(function mySucces(response) {
           ...
           $scope.registrar = function (event) {
               ...
               if ($scope.txtCredencial!=undefined &&  $scope.txtCredencial.trim().length > 0) {
                   ...
                   $http({
                       ...
                       data: {
                           codAutMov: codMovimiento, codDetAut: visitaseleccionada.cod_detalleAutorizacion, tipoMov: $scope.TipoIngreso,
                           numCred: $scope.txtCredencial, codAccPuesto: accesoSeleccionado.cod_acceso_puesto,
                           codPuesto: cod_puesto, obsMov: $scope.txtObservacion, codTipoDoc: visitaseleccionada.CodIde, dscDocIde: visitaseleccionada.DocIde, codVehi: cod_vehiculo, codEntidadIngresa: visitaseleccionada.cod_entidad
                       }
                   }).then(function mySucces(response) {
